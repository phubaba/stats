...
def get_f(r_t, h_i_tm1, lambda_i, gamma_i):
    return 1/numpy.sqrt(2*numpy.pi * h_i_tm1) * numpy.exp(-((r_t - (lambda_i + gamma_i * numpy.sqrt(h_i_tm1))) ** 2.) / (2 * h_i_tm1))

def get_f_i(delta_i_t, b_i, c_i):
    return numpy.abs(delta_i_t - b_i) - c_i * (delta_i_t - b_i)

def get_h_i_t(w_i, alpha_i, beta_i, b_i, c_i, mu_i, v_i, h_tm1_agg_i, delta_t_agg_i):
    if mu_i > 0:
        sqrthtpowmu = numpy.sqrt(h_tm1_agg_i) ** mu_i
        return (w_i + alpha_i * sqrthtpowmu * get_f_i(delta_t_agg_i, b_i, c_i) ** v_i + beta_i * sqrthtpowmu) ** (2./mu_i)
    else:
        sqrtht = numpy.sqrt(h_tm1_agg_i)
        return (numpy.exp(w_i + alpha_i * get_f_i(delta_t_agg_i, b_i, c_i)  ** v_i + beta_i * numpy.log(sqrtht)) ** (2.))

def get_P_t(d_1, e_1, r_t):
    return norm.cdf(d_1 + e_1 * r_t)

def get_Q_t(d_2, e_2, r_t):
    return norm.cdf(d_2 + e_2 * r_t)
...
def get_h_tm1_agg_i(p_1_tm1_agg_i, h_1_tm1, h_2_tm1, lambda1, gamma1, lambda2, gamma2):
    return (p_1_tm1_agg_i * h_1_tm1 + (1-p_1_tm1_agg_i) * h_2_tm1 +
        p_1_tm1_agg_i * (1-p_1_tm1_agg_i) * (lambda1 + gamma1*numpy.sqrt(h_1_tm1) - (lambda2 + gamma2*numpy.sqrt(h_2_tm1))) **2)

def get_delta_t_agg_i(p_1_tm1_agg_i, r_t, lambda_1, gamma_1, lambda_2, gamma_2, h_1_tm1, h_2_tm1):
    p1 =       p_1_tm1_agg_i * ((r_t- (lambda_1 + gamma_1 * numpy.sqrt(h_1_tm1)))/numpy.sqrt(h_1_tm1))
    p2 = (1 - p_1_tm1_agg_i) * ((r_t- (lambda_2 + gamma_2 * numpy.sqrt(h_2_tm1)))/numpy.sqrt(h_2_tm1))
    return p1 + p2

def get_p_1_tm1_agg_1(d_1, e_1, lambda_1, gamma_1, h_1_tm1, p_1_tm1, d_2, e_2, lambda_2, gamma_2, h_2_tm1, p_2_tm1):
    p1term =      norm.cdf(d_1+e_1*(lambda_1 + gamma_1 * numpy.sqrt(h_1_tm1)))*p_1_tm1
    p2term = (1 - norm.cdf(d_2+e_2*(lambda_2 + gamma_2 * numpy.sqrt(h_2_tm1))))*p_2_tm1
...
def get_p_1_tm1_agg_2(d_1, e_1, lambda_1, gamma_1, h_1_tm1, p_1_tm1, d_2, e_2, lambda_2, gamma_2, h_2_tm1, p_2_tm1):
    p1term = (1 - norm.cdf(d_1+e_1*(lambda_1 + gamma_1 * numpy.sqrt(h_1_tm1))))*p_1_tm1
    p2term =      norm.cdf(d_2+e_2*(lambda_2 + gamma_2 * numpy.sqrt(h_2_tm1)))*p_2_tm1
    return p1term/(p1term + p2term)
...
def loglikilihood(r, params1, params2, ):
    ...
    for t in xrange(1, len(r)):
        ...
        running_sum += numpy.log(f_1_t)
        running_sum += numpy.log(f_2_t)
